...
sb.run_backtester(  # Corrected method name
    stock='TSLA',
    interval='5 mins',  # interval of data to use for backtesting. check the folders in data/historical_data for available intervals and data. if not available get it from data\twelve_data.py
    days=1,  # number of days of say '5mins' data to use for backtesting. if interval is set to a day then there is a min of 30 days data so raise an error
    ...
    entry_strategies=[],  # When the previous elements and basic functionality have been fully tested then we can start implementing a dummy strategy and then move on to more complex strategies
