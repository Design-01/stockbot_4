...
class RandomOHLCV:
    ...
    open_rng  : tuple          # Range % change (min, max) from one bar to the next #* making min more neg than max gives a nore bias
    close_rng : tuple          # Range % change(min, max)  from one bar to the next
    ...
    def __post_init__(self) -> None:
        ...
        self.vol_rng = (self.vol_rng[0] * factor, self.vol_rng[1] * factor)
        ...
        vol_min = 50 * vol_factor
        vol_max = 500 * vol_factor
        ...
        for i in range(len(self.data.date)):
            ...
            v = int  (self.get_rand_val(v, self.vol_rng,   minVal=vol_min, maxVal=vol_max, div=1, volotility=1))

            v = min(max(50*vol_factor, v), 5000*vol_factor) # stop vol going excessively too high or low 
